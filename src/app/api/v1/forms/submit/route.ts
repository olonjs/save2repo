...
import { getGithubAppClient, resolveCorrelationId } from "@/lib/licensing";
...
type TenantRow = {
  ...
  forms_git_storage_enabled: boolean | null;
  forms_storage_policy: string | null;
...
async function commitLeadToGithub(params: {
  ...
  const owner = params.tenant.github_repo_owner;
  const repo = params.tenant.github_repo_name;
  const installationId = params.tenant.github_installation_id;
...
export async function POST(req: NextRequest) {
  ...
  const { data: tenant, error: tenantError } = await supabaseAdmin
    .from("tenants")
    .select(
      "id,name,slug,owner_id,github_installation_id,github_repo_owner,github_repo_name,forms_git_storage_enabled,forms_storage_policy,vercel_url"
    )
    .eq("api_key", apiKey)
    .single<TenantRow>();
  ...
  const owner = await supabaseAdmin.auth.admin.getUserById(tenant.owner_id);
  ...
  const installationId = tenant.github_installation_id;
  const repoOwner = tenant.github_repo_owner;
  const repoName = tenant.github_repo_name;
  if (tenant.forms_git_storage_enabled && installationId && repoOwner && repoName) {
    try {
  ...
  let storageMode = tenant.forms_storage_policy ?? "git_plus_db";
  let gitStorageAllowed = Boolean(tenant.forms_git_storage_enabled);
  ...
  if (runtimeRepoChecked && !runtimeRepoPrivate) {
    await supabaseAdmin
      .from("tenants")
      .update({
        forms_git_storage_enabled: false,
        forms_storage_policy: "db_only_public_repo",
      })
      .eq("id", tenant.id);
  }
  ...
  const notificationTemplate = await resolveTenantEmailTemplate({
    ...
    tenantRepo: {
      tenantId: tenant.id,
      tenantSlug: tenant.slug,
      installationId: tenant.github_installation_id,
